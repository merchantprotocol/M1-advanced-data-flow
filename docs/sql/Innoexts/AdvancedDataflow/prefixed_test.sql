...
DELETE FROM `[prefix]_eav_attribute` WHERE (`attribute_code` = 'base_currency') AND (`entity_type_id` = (
    SELECT `entity_type_id` FROM `[prefix]_eav_entity_type` WHERE `entity_type_code` = 'catalog_product'
));

DELETE FROM `[prefix]_core_resource` WHERE `code` = 'productbasecurrency_setup';

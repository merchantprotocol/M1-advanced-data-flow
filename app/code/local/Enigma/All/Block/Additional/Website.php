...
class Enigma_All_Block_Additional_Website extends Mage_Adminhtml_Block_Abstract{
    ...
    public function getTemplate(){
        return 'enigma_all/additional_website.phtml';
    }
    ...
    public function getRewrites(){
        if(null === $this->_rewrites){
            ...
            foreach($result as $rewritedPath => $_rewrites){
                ...
                $_classNames = array('enigma' => array(), 'other' => array());
                foreach($_rewrites as $_rewriteClass){
                    ...
                    if($this->isEnigmaModule($_rewriteClass)){
                        $_classNames['enigma'][] = $_rewriteClassHtml;
                    } else {
    ...
    public function isEnigmaModule($className){
        $_modulePath = explode('_', $className);
        if(count($_modulePath) != 0 && $_modulePath[0] == 'Enigma'){
            return true;
        }
        return false;
    }
    ...
    public function getDisabledEnigmaModules(){
        $result = array();
        foreach($this->getModulesArray() as $moduleName => $options){
            if(!$this->isEnigmaModule($moduleName)){
                continue;
            }

...
class Enigma_All_Block_System_Config_Form_Fieldset_Enigmaall_Extensions extends Mage_Adminhtml_Block_System_Config_Form_Fieldset{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element){
        ...
        foreach ($modules as $moduleName) {
            ...
            if($moduleName == 'Enigma_Core' || $moduleName == 'Enigma_All'){
                continue;
            }
    ...
    protected function _getFieldHtml($fieldset, $moduleName){
        ...
        if(!$ignore_platform){
            ...
            if($magentoVersion >= $this->_convertVersion(Enigma_All_Helper_Config::ENTERPRISE_VERSION)){
                // EE
            ...
            } elseif($magentoVersion >= $this->_convertVersion(Enigma_All_Helper_Config::PROFESSIONAL_EDITION)){
                // PE
                if($platform == 'ce'){
        ...
        $hasUpdate = false;
        if($displayNames = Mage::app()->loadCache('enigma_all_extensions_feed')){
            if($displayNames = unserialize($displayNames)){
                if(isset($displayNames[$moduleName])){
                    ...
                    if($warning){
                        $update = '<a  target="_blank"><img src="' . $this->getSkinUrl('enigma_all/images/bad.gif') . '" title="' . $this->__("Wrong Extension Platform") . '"/></a>';
                        $moduleName = "$update $moduleName";
                    } else {
                        if($this->_convertVersion($ver) < $this->_convertVersion($version)){
                            $update = '<a href="' . $url . '" target="_blank"><img src="' . $this->getSkinUrl('enigma_all/images/update.gif') . '" title="' . $this->__("Update available") . '"/></a>';
                            $hasUpdate = 1;
                            $moduleName = "$update $moduleName";
                        }
                    }
        ...
        if(!$hasUpdate && !$warning){
            $update = '<a  target="_blank"><img src="' . $this->getSkinUrl('enigma_all/images/ok.gif') . '" title="' . $this->__("Installed") . '"/></a>';
            $moduleName = "$update $moduleName";

...
class Enigma_All_Block_Jsinit extends Mage_Adminhtml_Block_Template{
    ...
    protected function _prepareLayout(){
        ...
        if($this->_section == 'enigmaall'){
            $this->getLayout()
                    ->getBlock('head')
                    ->addJs('enigma_all/enigma_all.js');
            $this->setData('extensions', $this->_initExtensions());
        } elseif($this->_section == 'enigmastore'){
            // Enigma extensions store
            $this->getLayout()
                    ->getBlock('head')
                    ->addJs('enigma_all/enigma_all.js');
    ...
    protected function _toHtml(){
        if($this->_section == 'enigmaall' || $this->_section == 'enigmastore'){
    ...
    protected function _initExtensions(){
        ...
        foreach($modules as $moduleName){
            ...
            $platform = Enigma_All_Helper_Versions::convertPlatform($platform);
    ...
    public function getIcon($Extension){
        if($Extension->getUpgradeAvailable()){
            $icon = 'enigma_all/images/update.gif';
            $title = "Update available";
        } elseif(!$Extension->getIsPlatformValid()){
            $icon = 'enigma_all/images/bad.gif';
            $title = "Wrong Extension Platform";
        } else {
            $icon = 'enigma_all/images/ok.gif';
    ...
    protected function _getStoreData(){
        ...
        if($storeResponse !== false){
            ...
            Mage::app()->saveCache($storeResponse, Enigma_All_Helper_Config::STORE_RESPONSE_CACHE_KEY);
        }
        else {
            $storeResponse =  Mage::app()->loadCache(Enigma_All_Helper_Config::STORE_RESPONSE_CACHE_KEY);
            if(!$storeResponse){
    ...
    protected function _getStoreConnection(){
        ...
        $url = rtrim(Enigma_All_Helper_Config::STORE_URL) . (sizeof($url) ? ("?" . implode("&", $url)) : "");
